...
export default {
  start(app) {
      ...
      switch (data.tag) {
        case "Reload":
          requestAnimationFrame(() => {
            document
              .querySelectorAll("ellie-ui-output[data-connected]")
              .forEach(el => {
                el.reload();
              });
          });
    ...
    const onReady = (doc, marker) => {
      const script = doc.createElement("script");
      script.textContent = `function __ellie_onReady_${marker}(cb) {
          function completed() {
        ...
        class extends HTMLElement {
          ...
          _onMessage(e) {
            switch (e.data.tag) {
              case "LogReceived":
                this.dispatchEvent(
                  new CustomEvent("log", { detail: e.data.contents })
                );
                return;
              case "Ready":
                this._canDebug = e.data.contents.canDebug;
                this.dispatchEvent(
                  new CustomEvent("canDebug", {
                    detail: e.data.contents.canDebug
                  })
                );
                this._onReady();
                return;
              case "RuntimeException":
                this.dispatchEvent(
                  new CustomEvent("runtimeException", {
                    detail: e.data.contents.message
                  })
                );
            }
          }
